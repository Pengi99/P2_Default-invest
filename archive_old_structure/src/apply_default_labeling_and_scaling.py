...
# 부실 기업 정보 로드
value_fail = pd.read_csv('data/processed/value_fail.csv', dtype={'종목코드': str})
...
# 라벨링된 데이터 저장
fs_ratio_final.to_csv('data/processed/FS_ratio_flow_labeled.csv', index=False, encoding='utf-8-sig')
print(f"✅ 라벨링된 데이터 저장: data/processed/FS_ratio_flow_labeled.csv")
...
with pd.ExcelWriter('data/processed/scaling_analysis.xlsx', engine='openpyxl') as writer:
...
print(f"✅ 스케일링 분석 결과 저장: data/processed/scaling_analysis.xlsx")
