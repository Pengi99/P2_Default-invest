"""
랜덤 포레스트 모델 - 100% 완성도 데이터용 Optuna 하이퍼파라미터 튜닝
"""
...
class RandomForestOptimizer:
    ...
    def objective(self, trial, X_train, y_train, cv_folds=5):
        ...
        # bootstrap이 False인 경우 oob_score는 사용할 수 없음
        if params['bootstrap']:
    ...
    def optimize(self, X_train, y_train, n_trials=100, cv_folds=5):
        ...
        self.study = optuna.create_study(
            direction='maximize',
            sampler=sampler,
            study_name='random_forest_100_optimization'
        )
...
def run_optimization(data_type='smote'):
    """모델 최적화 실행"""
    print(f"=== 100% 완성도 데이터 랜덤 포레스트 최적화 ({data_type.upper()}) ===")
    ...
    X_train = pd.read_csv(f'data/final/X_train_100_{data_type}.csv')
    X_valid = pd.read_csv(f'data/final/X_valid_100_{data_type}.csv')
    X_test = pd.read_csv(f'data/final/X_test_100_{data_type}.csv')
    
    y_train = pd.read_csv(f'data/final/y_train_100_{data_type}.csv').iloc[:, 0]
    y_valid = pd.read_csv(f'data/final/y_valid_100_{data_type}.csv').iloc[:, 0]
    y_test = pd.read_csv(f'data/final/y_test_100_{data_type}.csv').iloc[:, 0]
    ...
    # 모델 저장
    model_path = f'outputs/models/random_forest_100_{data_type}_{timestamp}.joblib'
    ...
    results_path = f'outputs/reports/random_forest_100_{data_type}_{timestamp}.json'
