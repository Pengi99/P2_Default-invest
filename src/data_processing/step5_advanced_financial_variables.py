...
def load_and_prepare_data():
    ...
    # FS_flow.csv 로드
    fs_flow = pd.read_csv('../../data/processed/FS_flow.csv', dtype={'거래소코드': str})
...
def calculate_stability_risk_variables(df):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        # 차입의존도 (이자보상배율을 이용해 추정)
        # 이자보상배율이 낮을수록 차입의존도가 높다고 가정
        # 차입의존도 = 1 / (1 + 이자보상배율) - 정규화된 지표
        df_sorted['차입_의존도'] = 1 / (1 + df_sorted['이자보상배율'].abs())
...
def calculate_valuation_cashflow_variables(df):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        # 현금흐름 기반 ROA
        df['현금흐름_기반_ROA'] = df['영업현금흐름'] / df['자산_평균']
    
    valuation_vars = ['이익수익률', '매출액_대비_현금흐름', '현금흐름_기반_ROA']
...
def main():
    ...
    try:
        ...
        print("💰 가치평가 (3개): 이익수익률, 매출액_대비_현금흐름, 현금흐름_기반_ROA")
